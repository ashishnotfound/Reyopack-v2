...
export function CsvImporter({ disabled }: { disabled: boolean }) {
  ...
  function selectFile(file: File | undefined) {
    ...
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) { setError("CSV files must be 5 MB or smaller."); return; }
    ...
    Papa.parse<ParsedRow>(file, {
      ...
      complete: ({ data, meta, errors }) => {
        ...
        setRows(data.slice(0, 5000).map((raw, index) => {
  ...
  async function confirmImport() {
    ...
    try {
      ...
      const response = await fetch("/api/admin/orders/import", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ rows: payload }) });
      const data = await response.json() as { imported?: number; duplicates?: number; error?: string };
      if (!response.ok) throw new Error(data.error ?? "Import failed.");
      setResult(`${data.imported ?? payload.length} orders imported. ${data.duplicates ?? 0} existing orders updated without duplication.`);
  ...
  return <div className="space-y-5">
    ...
    <Card><CardContent className="p-6"><label className="flex min-h-44 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed bg-muted/25 p-6 text-center hover:bg-muted/45"><FileSpreadsheet className="mb-3 size-10 text-primary" /><span className="font-semibold">Choose a CSV export</span><span className="mt-1 text-sm text-muted-foreground">Up to 5,000 rows and 5 MB</span><Input type="file" accept=".csv,text/csv" className="sr-only" onChange={(event) => selectFile(event.target.files?.[0])} /></label><div className="mt-3 flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-muted-foreground">Required: {requiredColumns.join(", ")}. Optional: awb, asin, variation.</p><Button asChild variant="outline" size="sm"><a href={`data:text/csv;charset=utf-8,${encodeURIComponent(template)}`} download="reyo-pack-orders-template.csv"><Download />Download template</a></Button></div></CardContent></Card>
