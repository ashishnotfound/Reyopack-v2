...
1. `/pack` keeps the scanner field focused and accepts an AWB, marketplace order ID, internal order number, or SKU.
2. `GET /api/orders/lookup` validates the scan and calls the indexed `lookup_order` database function.
...
The unique constraint on `packing_events.order_id` and row lock make concurrent confirmation idempotent. A later scan is still allowed and returns the original attribution without creating a second event.

## Application boundaries

- Server Components load authenticated operational data.
- Client Components are limited to scanner interaction, forms, local device preferences, and Realtime subscriptions.
